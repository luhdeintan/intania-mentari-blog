<template>
    <div>
        <div class="p-4 flex gap-4 header">
            <button class="btn">Hello daisyUI {{ colorMode.value }}</button>
            <select class="select w-full max-w-xs" v-model="colorMode.preference" data-theme="{{ colorMode.value }}">
                <option disabled selected data-theme="{{ colorMode.value }}">Theme</option>
                <option v-for="theme of themes" :key="theme" data-theme="{{ colorMode.value }}">{{ theme }}</option>
            </select>
        </div>
        ...
        <div>
		  ...
		  <footer className="footer footer-center p-10 bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 text-primary-content">
			<aside>
